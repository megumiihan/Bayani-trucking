...
export default function DeleteExpenseModal({
  ...
  const dateMatches = typedDate.trim() === expense.date;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        >
          {step === "preview" ? "Delete this expense?" : "Confirm permanent delete"}
        </h2>
        <p className="mt-1 text-sm text-gray-500">
          {step === "preview"
            ? "Review the entry below. Nothing has been deleted yet."
            : "This cannot be undone. The row will disappear from the expense log and CSV exports."}
        </p>
        ...
        <dl className="mt-5 space-y-2 rounded-lg border border-red-100 bg-red-50 px-4 py-3 text-sm">
          ...
          <PreviewRow
            label="Reimbursed"
            value={expense.reimbursed ? "Yes" : "No"}
          />
          <PreviewRow label="Credit" value={expense.credit ? "Yes" : "No"} />
          <PreviewRow label="Description" value={expense.description || "—"} />
        </dl>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          {step === "preview" ? (
            ...
            <button
              type="button"
              disabled={!dateMatches || isDeleting}
